...
Le FEATURE ENGINEERING est ici : on transforme chaque mot en nombres
qui decrivent ce qu'un humain regarderait (ou est-il ? contient-il
des chiffres ? est-il long ?).
"""
import json
import numpy as np
...
def featurize_word(text, cx, cy, img_w, img_h):
    ...
    return [
        cx / img_w,                      # position horizontale (0=gauche, 1=droite)
        cy / img_h,                      # position verticale (0=haut, 1=bas)
