...
Trois backends interchangeables. Le projet ne depend d'aucun fournisseur
en particulier : si l'un est indisponible (quota, region, panne), on bascule
sur un autre sans toucher au reste du code.
"""
import json
import re
...
def init_llm(backend="groq", api_key=None, model_name=None):
    """Initialise le backend choisi.

    backend="groq"   : API gratuite, quota genereux (console.groq.com)
    backend="gemini" : Google AI Studio (nouveau SDK google-genai)
    backend="local"  : petit modele instruct sur le GPU Colab, AUCUNE API
    """
...
def _ask(prompt):
    """Envoie un prompt au backend actif et retourne le texte brut."""
    if _client is None:
    ...
    if _backend == "groq":
        r = _client.chat.completions.create(
            ...
            temperature=0,          # deterministe : on veut de l'extraction, pas de la creativite
        )
    ...
    if _backend == "local":
        out = _client([{"role": "user", "content": prompt}],
                      do_sample=False)
...
def extract_merchant_date(receipt_text):
    ...
    Ces champs sont ABSENTS des annotations CORD : impossible de les
    apprendre en supervise. Le prompting est la seule voie.
    """
...
{receipt_text[:1500]}"""
    ...
    match = re.search(r"\{.*\}", cleaned, re.DOTALL)   # isole le JSON
    try:
